...
2. Create two frequency arrays of size `26`:
   - `count1` for characters in `s1`.
   - `count2` for the current window in `s2`.
3. Build the frequency count of `s1`.
4. Build the frequency count of the first window of `s2` having size `len(s1)`.
5. If both frequency arrays are equal, return `True`.
6. Slide the window through `s2`:
   - Add the new character entering the window.
   - Remove the old character leaving the window.
7. After every slide, compare both frequency arrays.
...
Comparing arrays takes O(26), which is constant.
Space Complexity: O(1)
Two arrays of size 26 are used.
'''